...
\subsection{View Re-computation vs Maintenance}
...
\newline
\begin{equation}
performance matrix in percent = (time taken to compute view table - incremental update)
		\frac(time taken to compute view table) * 100
\end{equation}
...
For $Join$ \& $Aggregation$, we create two base tables as it involves k-kf joins, an intermediate table and a view table. We insert 3.000 rows in both the base table, apply k-fk joins and calculate sum for those joins in the view table. We then run experiments to see how we can improve performance as we split our tables across multiple regions.
