...
\section{Views}
In RDBMS, a \emph{view} is defined as result set of a query. Views in relational database systems are generally created for the queries that needs to be accessed frequently and that involves multiple joins. Queries that involve joins are usually expensive, so to reduce the cost of an operation, queries involving joins and complex logics are pre calculated and saved as a view. Views are simply a \emph{SELECT} statements to fetch desired result sets and are given certain name and saved in database. Views can also hide a complexity of a query. In a large dataset, when a computation is required to fetch data from several tables involving complex business logic, all the complex business logic can be moved to a \emph{view}, and then just use \emph{SELECT} statement to get data from that view thus, hiding the complexity of a query. Views, on the other hand also provide some layer of security mechanism to our database table. We can create a view without the columns containing confidential information, and restrict access to the base table. We can then provide access to the view and carry out desired operation using that view. 

In relational database systems, views are widely used. However, there are also certain disadvantages of views. In a scenario, where base table is deleted or modified, the view of that table becomes useless. A new view has to be created for the base table and then queries can be redirected to the newly created view. In \emph{SQL} database, in most of the cases, views are periodically refreshed to maintain consistency with the base table. This might not be a problem for small applications containing few hundred rows or columns, but re-calculating views for every client request in large dataset can be a bottleneck for performance optimization. To overcome this bottleneck, a new approach called \emph{materialized view} is used.


\section{Materialized View}

Materialized view is defined as the database object that stores the result of a query in a table or a disk. Materialized views are widely used for optimizing performance of the system, i.e. to speed up query processing time over large datasets. The need for materialized view addresses the problem of having to query large datasets that often needs joins and aggregations between multiple tables. These kind of queries are very expensive regarding execution time and processing power. Materialized view speeds up the query processing time by pre-computing joins and aggregations before execution and stores these results in a table or disk \cite{incremental:materializedviews}. 

\section{Materialized View Maintenance}
Once the materialized views are created, our query is redirected to materialized view rather than the base table. Whenever there is an update in the base table, the materialized view also has to be updated accordingly. One of the solutions would be recomputing the entire materialized view from the scratch or using the heuristic of inertia \cite{maintenance:materializedviews} approach i.e. incremental maintenance with respect to the base table.
...
Recomputing materialized view from scratch every time there is an update on base table is expensive. The other approach is to update the part of materialized view with respect to the update in the base table. Our target is to maintain consistency between materialized view and base table whenever there is an update in the base table.
...
\begin{figure}
    ...
    \caption{Aggregation: Insert a row with new (key,value) pair}
...
\begin{figure}
    ...
    \caption{Aggregation: Update value for an existing key}
...
\begin{figure}
    ...
    \caption{Join and Aggregation: Insert a row with new (key,value) pair}
...
\begin{figure}
	...
	\caption{Aggregation experiment in pseudo-distributed mode}
...
\begin{figure}
	...
	\caption{Join and Aggregation experiment in pseudo-distributed mode}
...
\begin{figure}
	...
	\caption{Join and Selection experiment in pseudo-distributed mode}
...
\begin{figure}
	...
	\caption{Aggregation experiment in fully-distributed mode}
	\label{sec:AggrExpDist}
...
\begin{figure}
	...
	\caption{Join and Aggregation experiment in fully-distributed mode}
...
\begin{figure}
	...
	\caption{Join and Selection experiment in fully-distributed mode}
...
In this thesis we have implemented basic aggregation functions namely \emph{count, sum, min} and \emph{max}. However, there can be more complex aggregation functions like user defined aggregation function. We certainly want to implement user defined aggregations at some point of time in near future. Also, in our implementation, join operation is carried out based on key-foreign key implementation. We would also like to further research on different complex structures of joins, like join operation carried out on non-primary key.\newline \newline
...
\newline
We ran our experiment in a laptop with a primary operating system and two virtual machines acting as a multi-node cluster. It would be more interesting to run our experiment in a cluster of more than 20 nodes with a dataset of few hundred million rows and few million columns and analyze the results.
