...
In relational database systems, views are widely used. However, there are also certain disadvantages of views. In a scenarios where base table is deleted or modified, the view of that table becomes useless. A new view has to be created for the base table and then queries can be redirected to the view. In \emph{SQL} database, in most of the cases, views are periodically refreshed to maintain consistency with the base table. This might not be a problem for small applications containing few hundred rows or columns, but re-calculating views for every client request in large dataset can be a bottleneck for performance optimization. To overcome this bottleneck, a new approach called \emph{materialized view} is used.
