...
We have presented an approach to maintain consistency between the base tables and a view table incrementally using HBase coprocessor. We have provided a solution for incremental maintenance of view table without re-computing the view table for an updates on the base table. For every updates on the base table, we managed to trigger required updates on the view table using coprocessor and apply them in the view table. This solution addresses the problem of having read-write overhead for re-computing view table even for small changes in the base table. We introduced an intermediate table where all the values for a particular key were put in a single row. So, for every updates for a particular key on base table, we only needed to scan a single row in the intermediate table to determine the required updates for the view table. This architecture overcomes read overhead i.e. we only need to scan one particular row instead of scanning the entire base table to determine changes required for the view table. As we overcome read-write overhead, we managed to improve performance of the system up to 99\% in the best case scenario. We ran experiments on different dataset ranging from 10.000 rows to 350.000 rows. We tested our solution for different view types namely $Sum$, $Count$, $Min$, $Max$ and \emph{K-FK join} views. We performed several experiments on both pseudo-distributed mode and fully-distributed mode. In both environments with different dataset, we were able to optimize performance significantly. 
